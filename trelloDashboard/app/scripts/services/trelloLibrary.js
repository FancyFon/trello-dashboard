  ...
  trelloLibrary.service("trelloService", function($http, $q, $rootScope){
    ...
    var functions = {
      ...
      getUser: function() {
        ...
        Trello.get("members/me?token="+$rootScope.token, {fields: "username,fullName,avatarHash,url"}, function(me){
          deferred.resolve(me);
        });
      ...
      getMember: function(userId) {
         ...
         Trello.get("members/"+userId, {fields: "username,fullName,avatarHash,url"}, function(me){
             deferred.resolve(me);
         });
         deferred.promise.then(function(me){
              $rootScope.pendingRequests--;
      ...
      getOrganisations: function() {
        ...
        Trello.get("members/me/organizations?token="+$rootScope.token, {fields: "name,displayName,url,closed"}, function(organisations) {
      ...
      getBoards: function(organisationId) {
        ...
        if(organisationId == "my") {
          Trello.get("members/me/boards?token="+$rootScope.token, {lists:"open"}, function(boards) {
          ...
          Trello.get("organizations/"+organisationId+"/boards?token="+$rootScope.token, {lists:"open"}, function(boards) {
            deferred.resolve(boards);
          });
          deferred.promise.then(function(boards) {
            $rootScope.pendingRequests--;
      ...
      getListCards: function(board) {
        ...
        angular.forEach(board.lists, function(list, j) {
          ...
          Trello.get("lists/" + list.id + "/cards?token="+$rootScope.token, {members : "true"}, function(cards) {
